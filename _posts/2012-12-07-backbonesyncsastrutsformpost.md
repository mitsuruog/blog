...
comments: true
tags: [backbone, sastruts, seaser2]
categories: ["Blog", "API・バックエンド連携"]
---

[Backbone.js Advent Calendar 2012](https:https://www.adventar.org/calendars/15)と[青森アドベントカレンダー](https:https://aomori-web-advent-calendar-2012.blogspot.jp/2012/12/aomori-web-adevent-calendar-2012.html)17 日目の記事です。青森ネタは最後のほうに気持ち程度あります。
...
（ちなみに、JSON のライブラリは[jsonic](https:https://jsonic.sourceforge.jp/)を使用しています。）
...
そう、**backbone.emulateJSON = true**の出番です。（本当に[このパート](https:https://github.com/enja-oss/Backbone/blob/master/docs/Sync.md)翻訳しといて良かったな w。）
...
PROTO.Views.view = Backbone.View.extend({
  ...
  create: function () {
    ...
    https://Content-Typeを偽装
    Backbone.emulateJSON = true;
    var model = new PROTO.Models.model(param);
    https://リクエストパラメータはAjaxのdata属性で渡す
...
public class IndexAction {
  ...
  https://TODO ActionForm
  https://ActionFormでリクエストパラメータを受け渡し
  @Required
  public String name;
  @Required
  public String age;

  https://TODO エラーの戻りをJSON化
...
[https:https://github.com/mitsuruog/SAStruts-Backbone](https://github.com/mitsuruog/SAStruts-Backbone)
...
- ← 前日　[Node.js + WebSocket + Backbone.js のすすめ](https:https://takesy.cocolog-nifty.com/atico/2012/12/nodejs-websocke.html)[](https:https://www.blogger.com/)（@takeshy）
- → 後日　[おさわり。backbone の ajax 周りの処理とか。](https:https://1000ch.net/2012/12/18/AjaxOfBackbone/)（@1000ch）

**Aomori Advent Calendar 2012**

- ← 前日　[茄子的　今年一番ヒットしたツールは・・・](https:https://nasunoblog.blogspot.jp/2012/12/blog-post.html)（@nasunotw）
- → 後日　[なぜ Facebook は僕の中で今年一番ヒットしたのか？](https:https://aomori-web-advent-calendar-2012.blogspot.jp/2012/12/facebook.html)（@kzki）
